...
# program will attempt to find the assembly link in the viewer. If
# however, the organsim is not a bacteria, then the program will raise an
# error. Similiary if there is no assembly link the program will also
# throw an error.
...
def _find_url(urlPiece, soup, viewer_soup):
    try:
        ...
        if 'Bacteria' not in str(viewer_soup):
            raise ValueError(
                "The Nucleotide accession specified is not a Bacteria!")
...
def _get_viewer_soup_nuccore(accession):
    viewerNucleotide = requests.get(
        "https://www.ncbi.nlm.nih.gov/sviewer/viewer.fcgi?id=" +
        accession +
        "&db=nuccore&extrafeat=0&fmt_mask=0&retmod=html&withmarkup=on&tool=portal&log")
    return bs4.BeautifulSoup(viewerNucleotide.text)


def _get_soup_assembly(accession):
    the_request = requests.get(
        "https://www.ncbi.nlm.nih.gov/assembly/" + accession)
    return bs4.BeautifulSoup(the_request.text)

# Gets the HTML file for the NCBI accession number of interest. Utilizes
# commands instead of global variables to reduce semantic coupling!
...
# Finds the div element with a rprt class
    if not new_soup.find_all('a', attrs={'href': re.compile(
